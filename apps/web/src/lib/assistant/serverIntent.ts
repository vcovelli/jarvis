...
export type AssistantIntentResolution = {
  ...
  used: "fallback" | "openai" | "local";
  context: AssistantContextPayload;
};

export async function resolveAssistantIntent(params: {
  userId: string;
  input: string;
  context: unknown;
  ...
  const fallback = parseAssistantIntentFallback(params.input, context);
...
export function sanitizeAssistantContext(value: unknown): AssistantContextPayload {
...
function buildIntentSystemPrompt() {
  return [
    ...
    "mood can include mood 1-10, note, tags, day. Prefer tags from context.moodTags when they match the user wording.",
    "sleep can include durationMins, quality 1-5, recoveryScore 1-5, day, startMinutes, endMinutes, dreams, and notes. Map 'no dreams' to dreams: 'No dreams'.",
  ].join("\n");
}
